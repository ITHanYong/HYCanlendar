...
- (void)configureConstraintsForSubviews{
    ...
    if ([CalendarAppearance share].isShowLunarCalender) {
        ...
        dotY = CGRectGetMaxY(circleView.frame) - (CGRectGetMaxY(circleView.frame) - CGRectGetMaxY(lunarTextLabel.frame))/2;
    }
    else{
        textLabel.center = circleView.center;
        lunarTextLabel.hidden = YES;
        
        dotY = CGRectGetMaxY(circleView.frame) - (CGRectGetMaxY(circleView.frame) - CGRectGetMaxY(textLabel.frame))/2;
    }
    
    
    dotView.frame = CGRectMake(0, dotY, sizeDot, sizeDot);
    dotView.center = CGPointMake(self.frame.size.width / 2., dotY);
